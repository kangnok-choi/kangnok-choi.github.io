...
이 식은 물리적으로 매우 중요한 함의를 갖는다. 입력 신호 $f$의 주파수($p$)가 높아질수록 관측 데이터 $g_p$의 크기는 급격히 작아져 결국 0에 수렴하게 된다. 즉, $f$에서 $g$로 가는 매핑 과정에서 $f$의 고주파 성분(미세한 디테일)은 감쇄(damped)되고, 결과물인 $g$는 $f$보다 훨씬 부드러운 형태(smoothing)를 갖게 되는 것이다.
