...
여기에서, 함수 $u_i$와 $v_i$는 각각 left singular function, right singular function이라고 한다. 모든 $u_i$와 $v_i$ 내의 함수들은 식 (3)에서 정의한 inner product에 대해 orthonormal하다. 즉,


$$ \langle u_i, u_j \rangle = \langle v_i, v_j \rangle = \delta_{i, j}, \quad \text{for} \; i = 1, 2, \ldots $$

$\mu_i$는 singular value를 나타내며, 순서대로 monotonically decreasing하는 특성을 갖는다. 즉, 다음이 성립한다.
...
$f$와 $g$를 다음과 같이 확장할 수 있다. 

$$ f(t) = \sum_{i=1}^{\infty} \langle v_i, f \rangle v_i(t) \quad \text{and} \quad g(s) = \sum_{i=1}^{\infty} \langle u_i, g \rangle u_i(s) $$

적분 연산자 $A$를 활용해서 식 (6)의 fundamental relation을 표현하면 다음과 같이 간결하게 표현이 가능하다.

$$ A v_i = \mu_i u_i, \quad A^* u_i = \mu_i v_i, \quad i = 1, 2, \ldots $$

여기서 $A^*$는 앞서 언급한 adjoint kernel $K^*(t, s)$를 사용하는 수반 연산자(adjoint operator)이다. 이제 $g = Af$ 식에 앞서 정의한 $f$의 확장 식 $f = \sum_{i=1}^{\infty} \langle f, v_i \rangle v_i$를 대입하면 다음과 같은 관계를 얻는다.

$$ g = A \left( \sum_{i=1}^{\infty} \langle f, v_i \rangle v_i \right) = \sum_{i=1}^{\infty} \langle v_i, f \rangle (A v_i) = \sum_{i=1}^{\infty} \mu_i \langle v_i, f \rangle u_i $$

이 식을 $g$의 확장 식인 $g = \sum_{i=1}^{\infty} \langle u_i, g \rangle u_i$와 비교하면, 해의 계수와 데이터의 계수 사이의 관계가 도출된다.

$$ \langle u_i, g\rangle = \mu_i \langle v_i, f \rangle \quad \Rightarrow \quad \langle u_i , f \rangle = \frac{\langle u_i, g \rangle}{\mu_i} $$

따라서, Fredholm 적분 방정식의 최종적인 해 $f$는 다음과 같은 SVE expansion 형태로 나타낼 수 있다.

$$ f(t) = \sum_{i=1}^{\infty} \frac{\langle u_i, g \rangle}{\mu_i} v_i(t) $$

SVE로 유도한 해 $f(t)$가 물리적으로 존재(square integrable)하기 위해서는 해의 2-norm이 반드시 유한해야 한다. 이를 수식으로 나타내면 다음과 같은 Picard Condition을 얻는다.

\begin{equation} |f|_2^2 = \sum_{i=1}^{\infty} \left| \frac{\langle u_i, g \rangle}{\mu_i} \right|^2 < \infty \end{equation}

이 조건은 단순히 수학적인 수렴성을 넘어 inverse problem에서 매우 중요한 두 가지 정보를 제공한다.

1. **(Decay rate)** Picard condition은 데이터의 계수인 $|\langle u_i, g \rangle|$가 singular value $\mu_i$보다 더 빠르게 0으로 수렴해야 함을 의미함.

    * 이는 데이터 $g$가 충분히 smooth해야 함을 시사함. 

    * 시스템 $A$가 고주파 성분을 damping하는 속도 ($\mu_i$)보다 데이터가 가진 고주파 성분이 더 빨리 사라져야 inverse computation 시 발생하는 증폭을 견디고 유한한 크기의 해를 구성할 수 있음.

2. **Violation by Noise** 이론적으로 완벽한 데이터는 이 조건을 만족할 수 있지만, 실제 측정된 데이터 $g$에는 항상 노이즈가 섞여 있음. 이 노이즈가 inverse problem을 파괴하는 과정은 다음과 같음.

    * 이상적인 데이터: Noise가 없다면 $|\langle u_i, g \rangle|$은 $\mu_i$보다 빠르게 줄어들어 Picard condition을 만족함.

    * 데이터 계수 $|\langle u_i, g \rangle|$은 특정 지점 이후부터 줄어들지 않고 noise level에서 평탄하게 유지됨 $\to$ Picard condition 발산
