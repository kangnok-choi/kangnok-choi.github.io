...
description: >
  ...
  "새 논문 정리/분석 시작", "논문 글 만들어줘", "템플릿 불러와줘", "내 초안 검토하고
  보강해줘", "플레이스홀더 지워줘", "논문이랑 내가 쓴 거 맞는지 봐줘" 같은 요청에 사용.
  세 단계(① 새 글 세팅 ② 검토·보강 ③ PR)로 구성. 각 유저는 자기 브랜치에서 작업하고,
  문서를 쓰면 반드시 PR로 올린다.
---
...
1. **정보 확인**: 논문(제목·저자·연도·학회·arXiv 링크), 대상 위치, 유저(=브랜치)를 파악.
   ...
   - 없는 정보만 짧게 묻는다.
2. **브랜치 확인/생성** (대원칙대로).
3. **페이지 생성**: 루트의 `paper_template.md`를 복사해
   `docs/<대상>/<슬러그>.md` 로 만든다. 슬러그는 소문자 kebab (`<paper-slug>.md`).
...
3. **보강**: 빠진 핵심(방법 디테일, 정의, 수식, 실험 설정, 저자가 인정한 한계 등)을
   유저 구조에 맞춰 채운다. 비판적 사고 섹션은 유저 관찰을 원문 근거로 뒷받침.
4. **이미지 로컬화**: 외부 URL(특히 `github.com/user-attachments/assets/...`)은
